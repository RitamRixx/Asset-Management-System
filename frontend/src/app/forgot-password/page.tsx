...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Backend always returns a generic 202 whether or not the email
      // exists (see password_reset_service.py) — the UI must not branch
      // on anything that would turn this into an account-enumeration
      // oracle. Always show the same "sent" state on success.
      setSubmitted(true);
    } catch {
      // Same reasoning applies to failures: don't surface anything more
      // specific than a generic error here.
