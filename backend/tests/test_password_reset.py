...
def test_reset_password_with_valid_token(client, admin, monkeypatch):
    ...
    response = client.post(
        "/api/v1/auth/reset-password", json={"token": raw_token, "new_password": "NewStrongPass456!"}
    )
    ...
    new = client.post("/api/v1/auth/login", json={"email": user.email, "password": "NewStrongPass456!"})
...
def test_reset_token_cannot_be_reused(client, admin, monkeypatch):
    ...
    first = client.post("/api/v1/auth/reset-password", json={"token": raw_token, "new_password": "FirstStrong123!"})
    assert first.status_code == 200

    second = client.post("/api/v1/auth/reset-password", json={"token": raw_token, "new_password": "SecondStrong123!"})
